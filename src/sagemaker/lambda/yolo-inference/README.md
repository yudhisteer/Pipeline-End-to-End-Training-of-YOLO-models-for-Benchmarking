...
```bash
yolo-inference$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
yolo-inference$ sam local start-api
yolo-inference$ curl http://localhost:3000/classify_digit
```
...
```bash
yolo-inference$ sam logs -n InferenceFunction --stack-name "yolo-inference" --tail
```
...
```bash
sam delete --stack-name "yolo-inference"
```
