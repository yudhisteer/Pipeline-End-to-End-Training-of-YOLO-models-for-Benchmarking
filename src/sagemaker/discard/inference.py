...
def model_fn(model_dir):
    ...
    try:
        ...
        model_files = ['best.onnx', 'best.pt', 'last.pt', 'yolo.pt', 'model.pt'] ###(NEW) 
        ###(ORGINAL) model_files = ['best.pt', 'last.pt', 'yolo.pt', 'model.pt']
        model_path = None
        ...
        try:
            ...
            if model_path.endswith('.pt') and hasattr(model, 'model'):    ###(NEW)
   ###(ORGINAL) if hasattr(model, 'model'):
                model.model.eval()
