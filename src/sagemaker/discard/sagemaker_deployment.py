...
class SageMakerDeployment:
    ...
    def create_model(self) -> PyTorchModel:
        ...
        self.model = PyTorchModel(
            ...
            source_dir="src/sagemaker", ###(NEW)
            ###(NEW) entry_point="src/sagemaker/serve/inference.py", ###(NEW) 
            ###(ORGINAL)entry_point="src/sagemaker/inference_aws.py",
            ###(ORGINAL) source_dir="src/sagemaker",
            ###(NEW) dependencies=["requirements.txt"], ###(NEW)
            ###(ORGINAL) dependencies=["src/sagemaker/dependencies/requirements.txt"],
    ...
    def deploy_endpoint(self, 
        ...
        instance_type = instance_type or endpoint_config.get('instance_type', 'ml.t2.medium') ###(NEW) 
        ###(ORGINAL) instance_type = instance_type or endpoint_config.get('instance_type', 'ml.m4.xlarge')
...
def deploy_model(job_name: str = None, config: Dict[str, Any] = None) -> SageMakerDeployment:
    ...
    instance_type = deployment_config.get('instance_type', 'ml.t2.medium') ###(NEW)
    ###(ORGINAL) instance_type = deployment_config.get('instance_type', 'ml.m5.large')
    ...
    # 1) Eğer config’ten model_data geldiyse, training job aramaya gerek yok
    if deployment.model_data:
        console.print(f"[cyan]Using model_data from config: {deployment.model_data}[/cyan]")
        # create_model -> self.model_data'yı kullanır
